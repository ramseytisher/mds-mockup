...
export default ({
  ...
  const { searchValue } = useContext(MdsContext)
  ...
  useEffect(() => {
    ...
    if (searchValue === "") {
      ...
      if (values.search(searchValue.toLowerCase()) >= 0) {
        setShowField(true)
      } else {
        setShowField(false)
      }
  ...
  if (fieldData && showField) {
    return (
      <>
        {divider && <FormDivider text={divider} />}
            ...
            {showFlag && (
              <Row style={{ padding: "4px" }}>
                <Card title={`Rule Name Here`} size="small">
                  <pre>
                    {JSON.stringify(fieldData.intelligenceData, null, 2)}
                  </pre>
                </Card>
              </Row>
            )}
